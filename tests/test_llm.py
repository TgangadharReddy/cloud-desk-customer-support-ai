...
All Gemini API calls are mocked — no real network requests or a real
API key are needed to run these tests.
"""
...
from app import llm
from app.llm import LLMGenerationError, NO_CONTEXT_RESPONSE, build_prompt, generate_response


class FakeFAQ:
    """
    A minimal stand-in for app.rag.RetrievedFAQ, so these tests don't
    take a hard dependency on the RAG module's (heavier) imports.
    """
...
def fake_settings(monkeypatch):
    """
    Every test gets a usable fake Gemini API key, without mutating the
    real (frozen) Settings object from app.config — we rebind the
    module-level `settings` name inside app.llm instead.
    """
    monkeypatch.setattr(
        llm,
        "settings",
        SimpleNamespace(gemini_api_key="fake-test-key", gemini_model="gemini-1.5-flash"),
    )


# --- Prompt construction (no API call involved) -----------------------------
...
def test_build_prompt_handles_empty_retrieval_without_error():
    prompt = build_prompt("What will the weather be tomorrow?", [])
...
def test_generate_response_returns_fallback_when_no_faqs_retrieved():
    result = generate_response("What will the weather be tomorrow?", [])
    assert result == NO_CONTEXT_RESPONSE


def test_generate_response_does_not_call_gemini_when_no_faqs_retrieved():
    with patch("app.llm.genai.GenerativeModel") as mock_model_cls:
        generate_response("What will the weather be tomorrow?", [])
...
def test_generate_response_returns_gemini_text_on_success(sample_faqs):
    ...
    mock_model.generate_content.return_value = SimpleNamespace(
        text="It looks like a duplicate charge. Please share both transaction IDs so billing can verify and refund it."
    )
    with patch("app.llm.genai.GenerativeModel", return_value=mock_model), patch(
        "app.llm.genai.configure"
    ):
        result = generate_response("I was charged twice.", sample_faqs)

    assert result == (
        "It looks like a duplicate charge. Please share both transaction "
        "IDs so billing can verify and refund it."
    )
...
def test_generate_response_raises_on_api_error(sample_faqs):
    with patch(
        "app.llm.genai.GenerativeModel", side_effect=RuntimeError("network error")
    ), patch("app.llm.genai.configure"):
        with pytest.raises(LLMGenerationError):
            generate_response("I was charged twice.", sample_faqs)
